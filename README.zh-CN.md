...
MarketSentinel 用于在 A 股交易时段监控指定场内 ETF 的 IOPV 溢价率。程序优先通过东方财富的定向行情接口获取已配置 ETF 的行情；如该接口不可用，则回退至 AKShare。当溢价满足你设定的阈值时，向飞书（Lark）自定义机器人 Webhook 发送提醒。
...
2. 优先从东方财富定向读取已配置 ETF 的行情，必要时回退至 AKShare。
...
- 可访问东方财富/AKShare 上游行情和飞书/Lark 的网络。
...
本项目优先通过东方财富公开行情接口读取已配置的 ETF 代码；[AKShare](https://github.com/akfamily/akshare) 作为兼容回退。两者均依赖公开上游数据，请自行确认使用条款和适用性。
